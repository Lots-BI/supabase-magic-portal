...
function SyncStatusSkeleton() {
  return <div className="lots-skeleton h-24 w-full rounded-xl" />;
}
...
function SyncStatusBody({ queryName }: { queryName: string }) {
  ...
  if (!meta && health.platforms.length === 0) {
    return (
      <div className="lots-surface flex items-start gap-3 border-warning/30 bg-warning/5 p-4">
  ...
  return (
    <div
      className={cn(
        "lots-surface relative isolate z-0 p-4",
...
function PlatformHealthRow({ platform }: { platform: PlatformHealth }) {
  return (
    <div className="flex items-start gap-2 rounded-lg border border-border bg-background/50 px-2.5 py-2">
      <span
