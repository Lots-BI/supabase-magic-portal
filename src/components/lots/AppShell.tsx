...
import { LotsBIWordmark } from "./LotsMark";
...
export function AppShell({
  ...
  return (
    <div className="lots-app-shell min-h-screen min-h-[100dvh] overflow-x-hidden bg-background text-foreground">
      <a
        href="#main-content"
      ...
      >
        <div className="flex h-14 shrink-0 items-center px-5 pt-[env(safe-area-inset-top)]">
          <Link to="/" className="lots-focus rounded-md">
            <LotsBIWordmark />
          </Link>
        </div>
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        ...
        >
          ...
          <div className="flex h-14 shrink-0 items-center border-b border-sidebar-border px-5">
            <Link to="/" className="lots-focus rounded-md" onClick={closeMobile}>
              <LotsBIWordmark />
            </Link>
          </div>
      ...
      {/* Mobile header */}
      <header className="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/90 lg:pl-[248px]">
        <div
          className={cn(
        ...
        >
          ...
          <Link
            to="/"
            className="lots-focus min-w-0 shrink truncate rounded-md lg:hidden"
            aria-label="Início"
          >
            <LotsBIWordmark className="max-w-[140px] sm:max-w-none" />
          </Link>
      ...
      <main className="lg:pl-[248px]" id="main-content">
        ...
        >
          <div className="animate-lots-fade min-w-0">{children}</div>
        </div>
      </main>
    </div>
  );
