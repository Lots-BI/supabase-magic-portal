...
import { SyncStatusBar } from "@/components/lots/SyncStatusBar";
    ...
    <div className="space-y-8">
      ...
      {sections.length === 0 ? (
        ...
        sections.map((section, index) => (
          <section
            key={section.family}
            className={cn("space-y-3", index > 0 && "border-t border-border pt-8")}
          >
  ...
  if (single && account) {
    return (
      <Link
        ...
        className={cn(
          "lots-surface lots-focus group relative isolate flex min-h-[8.25rem] flex-col overflow-hidden p-4 sm:min-h-[8.75rem] sm:p-5",
        ...
        {group.accounts.map((item) => (
          <li key={item.slug}>
            <Link
              ...
              className="lots-focus group flex min-h-[44px] items-center gap-3 px-3 py-2.5"
            >
