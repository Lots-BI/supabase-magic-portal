...
import { syncGoogleAdsCampaignsConnection } from "./google-ads-campaigns-sync.server";
...
export const syncGoogleAdsCampaignsFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: connection, error } = await getSupabaseAdmin()
      ...
      .eq("plugin_key", "google_ads")
      .eq("status", "active")
      .maybeSingle();
    if (error) throw new Error(error.message);
    if (!connection?.id) {
      return { ok: false, error: "Google Ads não conectado para este cliente" };
    }

    const result = await syncGoogleAdsCampaignsConnection(getSupabaseAdmin(), connection.id);
    if (!result.ok) {
