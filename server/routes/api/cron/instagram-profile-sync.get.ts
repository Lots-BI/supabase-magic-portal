...
/** Cron diário — Insights de perfil Instagram (todas as conexões ativas). */
export default defineEventHandler(async (event) => {
  assertCronAuth(event);

  const startedAt = new Date().toISOString();
  const supabase = getSupabaseAdmin();
  const summary = await syncAllInstagramProfileConnections(supabase);

  if (summary.failed > 0) {
    const stack = await createAdminHubStack(supabase);
    await stack.timeline.append({
      kind: "sync_failed",
      title: `Cron Instagram perfil: ${summary.failed} falha(s)`,
